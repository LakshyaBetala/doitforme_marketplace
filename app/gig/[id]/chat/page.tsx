  ...
  return (
    <div className="flex flex-col h-screen bg-[#0B0B11] text-white selection:bg-brand-purple">
      ...
      <div className="p-4 border-b border-white/10 bg-[#121217] flex items-center gap-4 shadow-md z-10">
        <Link href={`/gig/${id}`} className="p-2 hover:bg-white/10 rounded-full transition text-white/50 hover:text-white">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[url('/grid-pattern.svg')] bg-repeat opacity-90">
        ...
        {/* PRIVACY & LIMIT BANNER */}
        <div className="text-center py-6">
          {!isEscrowSecured ? (
            <div className="mb-4 space-y-2 animate-in zoom-in-95">
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-brand-purple/10 border border-brand-purple/20 rounded-full text-brand-purple text-xs font-bold">
                <ShieldCheck className="w-3 h-3" /> Limited Chat Active
              </div>
              <p className="text-white/50 text-xs max-w-xs mx-auto">
                Ask clarifying questions. <br />
                <span className="text-white font-bold">{messagesRemaining} messages remaining</span> until deal is accepted.
              </p>
            </div>
          ) : (
            <div className="mb-4">
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-500/10 border border-green-500/20 rounded-full text-green-400 text-xs font-bold">
                <Check className="w-3 h-3" /> Handshake Guarantee Active
              </div>
            </div>
          )}

          <div className="p-3 bg-red-500/5 border border-red-500/10 rounded-xl max-w-xs mx-auto">
            <p className="text-[10px] text-red-300/60 leading-tight">
              Deals closed on WhatsApp are <b>NOT protected</b> by the DoItForMe Handshake Guarantee.
            </p>
          </div>
        </div>
      ...
      <div className="p-4 bg-[#121217] border-t border-white/10">
        <form onSubmit={sendMessage} className="flex gap-3 max-w-4xl mx-auto">
          ...
          >
            {isChatLocked ? <Lock className="w-5 h-5" /> : <Send className="w-5 h-5" />}
          </button>
        </form>
      </div>
