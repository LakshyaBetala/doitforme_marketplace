...
export default function Dashboard() {
  ...
  return (
    <main className="min-h-[100dvh] bg-background text-foreground pb-20 font-sans selection:bg-brand-purple selection:text-white overflow-x-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 pt-6 md:pt-10">
        ...
        <section className="mb-8 md:mb-12 grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6">
           ...
           {/* Welcome & Stats */}
           <div className="md:col-span-8 p-6 md:p-8 rounded-[32px] bg-[#0A0A0A] border border-white/5 relative overflow-hidden group">
           ...
           <div className="md:col-span-4 flex flex-col gap-3">
              ...
              <div className="flex-1 grid grid-cols-2 gap-3">
                  <Link href="/gig/my-gigs" className="flex flex-col justify-center p-5 rounded-[24px] bg-[#0A0A0A] border border-white/5 hover:border-zinc-700 active:scale-[0.95] transition-all touch-manipulation group">
                  ...
                  <Link href="/gig/applied" className="flex flex-col justify-center p-5 rounded-[24px] bg-[#0A0A0A] border border-white/5 hover:border-zinc-700 active:scale-[0.95] transition-all touch-manipulation group">
        ...
        <section>
           ...
           <div className="sticky top-[64px] z-20 bg-background/95 backdrop-blur-xl py-4 mb-4 border-b border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <h2 className="text-lg md:text-xl font-black text-white flex items-center gap-2">
                  Live Gigs <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></span>
              </h2>
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                  <div className="relative group w-full sm:w-auto">
                    ...
                    <input 
                      ...
                      className="pl-9 pr-4 py-2.5 bg-[#0A0A0A] border border-white/10 rounded-full text-base md:text-sm text-white placeholder:text-zinc-600 focus:outline-none focus:border-brand-purple/50 w-full sm:w-48 transition-all"
                    />
                  </div>
                  {/* VIEW ALL BUTTON - Redirects to Feed */}
                  <Link href="/feed" className="p-2.5 px-4 rounded-full border border-white/10 hover:bg-white/5 text-zinc-400 hover:text-white transition-colors text-[10px] font-black uppercase tracking-widest whitespace-nowrap active:scale-95 touch-manipulation">
                     View All
                  </Link>
              </div>
           </div>
           ...
           <div className="grid grid-cols-1 gap-3 [content-visibility:auto]">
              {filteredGigs.length > 0 ? (
                filteredGigs.map((gig) => (
                  <Link key={gig.id} href={`/gig/${gig.id}`} className="group block active:scale-[0.99] touch-manipulation transition-transform">
                     <div className="relative p-5 rounded-[24px] bg-[#0A0A0A] border border-white/5 hover:border-zinc-600 hover:bg-[#0f0f0f] transition-all duration-300">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                           
                           {/* Left: Info */}
                           <div className="flex items-start gap-4">
                              <div className="w-12 h-12 rounded-xl bg-zinc-900 border border-white/5 flex items-center justify-center text-lg font-black text-zinc-500 group-hover:text-white group-hover:border-brand-purple/30 transition-all shrink-0">
                                 {gig.title[0]}
                              </div>
                              <div className="overflow-hidden">
                                 <h3 className="font-bold text-zinc-200 group-hover:text-white transition-colors text-base md:text-lg line-clamp-1">{gig.title}</h3>
                                 <div className="flex items-center gap-3 mt-1 text-[10px] text-zinc-500 font-bold uppercase tracking-wider">
                                    ...
                                    <span className="flex items-center gap-1">
                                       <Clock size={10} /> {timeAgo(gig.created_at)}
                                    </span>
                                 </div>
                              </div>
                           </div>
                           ...
                           {/* Right: Price & Action */}
                           <div className="flex items-center justify-between sm:justify-end gap-6 w-full sm:w-auto pl-16 sm:pl-0 border-t sm:border-0 border-white/5 pt-3 sm:pt-0">
                              <div className="text-left sm:text-right">
                                 <div className="text-xl font-black text-white group-hover:text-green-400 transition-colors">₹{gig.price}</div>
                                 <div className="text-[9px] text-zinc-600 uppercase tracking-widest font-black">Fixed Price</div>
                              </div>
                 ...
                 <div className="py-20 md:py-32 text-center border border-dashed border-zinc-800 rounded-[32px] bg-white/5 px-4">
      ...
      {activeChats.length > 0 && (
        <div className="fixed bottom-6 right-6 md:bottom-8 md:right-8 z-50 flex flex-col items-end gap-4">
          {isChatListOpen && (
            <div className="bg-[#0A0A0A] border border-zinc-800 rounded-2xl shadow-2xl p-4 w-[calc(100vw-48px)] max-w-[320px] mb-2 animate-in slide-in-from-bottom-5 fade-in duration-200">
